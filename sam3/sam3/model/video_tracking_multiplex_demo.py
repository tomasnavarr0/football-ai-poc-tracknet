...
class VideoTrackingMultiplexDemo(VideoTrackingDynamicMultiplex):
    ...
    def propagate_in_video(
        ...
        if len(output_dict["cond_frame_outputs"]) == 0:
            H_mask = W_mask = getattr(self, "low_res_mask_size", 288)
            pred_masks = torch.zeros(batch_size, 1, H_mask, W_mask, device=inference_state["device"], dtype=torch.float32)
            obj_scores = torch.full((batch_size, 1), -10.0, device=inference_state["device"], dtype=torch.float32)
            target_frame = start_frame_idx if start_frame_idx is not None else 0
            storage_device = inference_state.get("storage_device", inference_state["device"])
            compact_current_out = {
                "maskmem_features": None,
                "maskmem_pos_enc": None,
                "image_features": None,
                "image_pos_enc": None,
                "pred_masks": pred_masks.to(storage_device),
                "obj_ptr": None,
                "object_score_logits": obj_scores.to(storage_device),
                "conditioning_objects": [],
                "local_obj_id_to_idx": deepcopy(inference_state["obj_id_to_idx"]),
            }
            output_dict["non_cond_frame_outputs"][target_frame] = compact_current_out
            self._add_output_per_object(inference_state, target_frame, compact_current_out, "non_cond_frame_outputs")
            yield target_frame, obj_ids, pred_masks, None
            return
        clear_non_cond_mem = self.clear_non_cond_mem_around_input and (
            self.clear_non_cond_mem_for_multi_obj or batch_size <= 1
        )
        ...
        if add_to_existing_state or reconditioning:
            with torch.profiler.record_function(
                ...
                if existing_out is None:
                    H_mask = W_mask = getattr(self, "low_res_mask_size", 288)
                    storage_device = inference_state.get("storage_device", inference_state["device"])
                    cur_obj_num = self._get_obj_num(inference_state)
                    existing_out = {
                        "maskmem_features": None,
                        "maskmem_pos_enc": None,
                        "image_features": None,
                        "image_pos_enc": None,
                        "pred_masks": torch.zeros(cur_obj_num, 1, H_mask, W_mask, device=storage_device, dtype=torch.float32),
                        "obj_ptr": None,
                        "object_score_logits": torch.full((cur_obj_num, 1), -10.0, device=storage_device, dtype=torch.float32),
                        "conditioning_objects": [],
                        "local_obj_id_to_idx": deepcopy(inference_state["obj_id_to_idx"]),
                    }
                    output_dict["non_cond_frame_outputs"][frame_idx] = existing_out
...
class Sam3VideoTrackingMultiplexDemo(VideoTrackingMultiplexDemo):
    ...
    def propagate_in_video(
        ...
        if len(output_dict["cond_frame_outputs"]) == 0:
            ...
            output_dict["non_cond_frame_outputs"][target_frame] = compact_current_out
            self._add_output_per_object(inference_state, target_frame, compact_current_out, "non_cond_frame_outputs")
            yield target_frame, obj_ids, pred_masks, None, obj_scores
            return
        clear_non_cond_mem = self.clear_non_cond_mem_around_input and (
            self.clear_non_cond_mem_for_multi_obj or batch_size <= 1
        )
